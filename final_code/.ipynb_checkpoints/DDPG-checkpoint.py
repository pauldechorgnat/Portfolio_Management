...
class ActorNetwork(object):
    def __init__(self, sess, state_size, action_size, BATCH_SIZE, TAU, 
                 LEARNING_RATE=0.01, HIDDEN1_UNITS = 30, HIDDEN2_UNITS = 60):
        self.sess = sess
...
class DDPGAgent:
    def __init__(self, hidden_units1 = 100, hidden_units2 = 2, 
                 number_of_assets = DEFAULT_NUMBER_OF_ASSETS,
                 buffer_size = 10, batch_size = 5,
                 learning_rate = .01, GAMMA = .5):
        """Init a new agent.
    ...
    def act(self, observation):
        ...
        if self.time <= self.buffer_size:
            print('not enough value to take an action')
            # returning no investment actions
            null_action = np.zeros(self.number_of_assets+1)
            null_action[-1] = 1
            print(null_action)
            return null_action
        else:
    ...
    def reward(self, observation, action, reward):
        ...
        if self.time > 1:
            print("we are going to add the stuff")
            ...
            if self.time > self.buffer_size + 1:
                print("\t"*100 +str(self.time))
                # learning time
                ...
                y_t = self.GAMMA * target_q_values + y_t
                # 
                loss += self.critic_network.model.train_on_batch([states,actions], y_t) 
                a_for_grad = self.actor_network.model.predict(states)
